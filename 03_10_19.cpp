...
//const int C_size   =  5;
const int C_poison = -8;                                                                                                                                                                                                                      const int C_secret = 179;
const int C_start_capacity = 10;
...
bool Stack_Realloc  (stack_t* this_);  // no
...
long int Stack_Control_Sum (const stack_t* const this_, long int sum);

bool Assert_Ok (const stack_t* const this_, const int mode = 0);
...
stack_t * Stack_Constructor ()
{
    ...
    res_ptr = (stack_t *) ((elem_t *)calloc (sizeof (struct stack_t) + C_start_capacity, sizeof(elem_t)));
...
//    (res_ptr -> saver_right) = C_saver_right;

//    if (!Assert_Ok (res_ptr))
//    {
//        PR_B(ERROR, Red)
//        printf (" ");
//        PR_B(BEFORE Stack_Constructor\n\n, Green)
//
//        return nullptr;
//    }
...
bool Stack_Full (const stack_t* const this_)
{
    ...
    return ((this_ -> size) == (this_ -> capacity) - 1);
}
...
long int Sum_Of_Struct (const stack_t* const this_)
{
    char *helper_ptr = (char *) this_;
    long int res = 0;

    for (int i = 0; i < sizeof (struct stack_t) + (this_ -> capacity); i++)
    {
        res += (int) *(helper_ptr + i);
//        printf ("res = [%li], som = (%d)\n", res, (int) *(helper_ptr + i));
    }
...
bool Assert_Ok (const stack_t* const this_, const int mode)
{
    bool res = true;

    if (mode == 0)
    {
        if (!Stack_Control_Sum (this_, (this_ -> hash)))
        {
